<template>
  ...
  <ul id="myUL">
    <!-- v-bind:key="i"는 순서(index)를 key로 쓰겠다 -->
    <li
    ...
    >
      <!-- v-on:click.stop 이 클릭 이벤트를 상위(부모)로 전파하지 말고 여기서 멈추라 -->
      {{ todo.title
      }}<span v-on:click.stop="removeTodo(i)" class="close">✖</span>
    </li>
  </ul>
</template>
...
export default {
  ...
  methods: {
    ...
    removeTodo(idx) {
      console.log(idx);
      console.log(this.todoList[idx].title);
